...
class Games {
    ...
    QuicksortListaDupla classe = new QuicksortListaDupla();
...
class Lista {
    ...
    private void quicksort(int esq, int dir) {
        ...
        while (i <= j) {
            while (array[i].getRelease_date().compareTo(pivo.getRelease_date()) < 0)
                i++;
            while (array[j].getRelease_date().compareTo(pivo.getRelease_date()) > 0)
                j--;
            if (i <= j) {
                swap(i, j);
                i++;
...
class CelulaDupla {
     ...
     */
    public CelulaDupla(Games elemento) {
