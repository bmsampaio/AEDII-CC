...
class Lista {
    ...
    public void ordena(int esq, int dir) {
        ...
        if (esq < dir && esq >= 0 && dir < array.length && array.length != 0) {
          int meio = ((dir + esq) / 2);
          ordena(esq, meio);
          ordena(meio + 1, dir);
     ...
     */
    private void mergesort(int esq, int meio, int dir) {
        Personagens[] temp = new Personagens[n];
        ...
        while (i <= meio && j <= dir) {
            if (temp[i].getHomeworld().compareTo(temp[j].getHomeworld()) <= 0) {
                array[k] = temp[i];
                i++;
            } else {
                array[k] = temp[j];
    ...
    public void desempate() {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = i + 1; j < n; j++) {
                if (array[i].getHomeworld().compareTo(array[j].getHomeworld()) == 0) {
