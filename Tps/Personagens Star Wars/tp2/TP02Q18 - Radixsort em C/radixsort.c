...
void sort()
{
    ...
    int i;
    Personagem *x;
    int maior = strlen(personagens[0].homeworld);
    int exp = 1;

    x = (Personagem *)calloc(n, sizeof(Personagem));

    for (i = 0; i < n; i++)
    {
        if (strlen(personagens[i].homeworld) > maior)
            maior = strlen(personagens[i].homeworld);
    }
    ...
    while (maior / exp > 0)
    {
        int b[10] = {0};
        for (i = 0; i < n; i++)
            b[(strlen(personagens[i].homeworld) / exp) % 10]++;
        for (i = 1; i < 10; i++)
            b[i] += b[i - 1];
        for (i = n - 1; i >= 0; i--)
        {
            x[--b[(strlen(personagens[i].homeworld) / exp) % 10]] = personagens[i];
        }
        for (i = 0; i < n; i++)
        {
            personagens[i] = x[i];
