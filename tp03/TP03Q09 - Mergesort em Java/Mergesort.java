...
class Games {
    ...
    Mergesort classe = new Mergesort();
...
class Lista {
     ...
     * os demais elementos para o esq da mesma.
     * 
    ...
    public void intercalar(int esq, int meio, int dir) {
        ...
        Games novoVetor[] = new Games[dir - esq];
        ...
        while (i < meio && m < dir) {
             ...
             */
            if (array[i].getUpvotes() <= array[m].getUpvotes()) {
...
public class Mergesort {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < numEntrada; i++) {
            games = new Games();
            ler(entrada[i]);
             System.out.println(games.getUpvotes());
            lista.inserirFim(games);
        } // end for
        lista.sort();
        for (int i = 0; i < lista.n; i++) {
            Games games = lista.array[i];
            System.out.print("[" + i + "] " + (lista.array[i].imprimir()) + "\n");
        }
        br.close();
    }// end main
