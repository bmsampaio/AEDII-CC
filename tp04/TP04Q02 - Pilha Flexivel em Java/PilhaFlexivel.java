...
class Games {
    ...
    PilhaFlexivel classe = new PilhaFlexivel();
...
public class PilhaFlexivel {
    ...
    public static void ler(String id) throws Exception, NumberFormatException {
        ...
        if (auxDev.contains("\",\"h") || auxDev.contains(",h")) {
            dev = (auxDev.split("http")[1]).split(",")[7];
        } else if (auxDev.contains("\",\"")) {
            dev = (auxDev.split("\",\"")[0]).split("\"")[1];
        } else {
            dev = (linha.split("\\]")[1]).split(",")[8];
        }
        if (dev.contains("\"")) {
            dev = dev.replace("\"", "");
        }
        games.setDevelopers(dev);
...
// how to creat a list of objects in java ?
